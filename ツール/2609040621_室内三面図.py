...
# 出典：確定事項ファイル D00-001〜013 / 部屋.xlsx / U05-001 暫定値
# 注意：冒頭の定義部は確定事項ファイルの写しである。再生成の前に定義部と
#       確定事項ファイルを突き合わせること（最初に読むこと 6-6）。
# 使用法：python3 室内三面図.py YYMMDDhhmm 'YYYY年M月D日 HH:MM'
...
LEG_BASE=[(FIX,'実線・通常表記 ＝ 確定値（確定事項ファイル D00-xxx を併記）'),
          (PROV,'破線・括弧付き表記 ＝ 推定・仮置き値（U05-001 暫定値）'),
          (CURC,'青実線 ＝ 部屋.xlsx の実測値（確定事項ファイル未登録）'),
          (AREAC,'橙ハッチ ＝ 機器設置エリア（梁1と梁2の間／U05-004）')]
...
open('/mnt/user-data/outputs/%s_F001_設置スペース平面図.svg'%ts,'w',encoding='utf-8').write(
 f.out('F001','設置スペース平面図','投影：−h方向を見た上面図（x−d平面）／原点を左上に置き x を右、d を下として描く',
...
open('/mnt/user-data/outputs/%s_F002_設置スペース側面図.svg'%ts,'w',encoding='utf-8').write(
 f.out('F002','設置スペース側面図','投影：x=146 の断面を +x 方向に見た側面図（d−h平面）／d を右、h を上として描く',
...
open('/mnt/user-data/outputs/%s_F003_設置スペース正面図.svg'%ts,'w',encoding='utf-8').write(
 f.out('F003','設置スペース正面図','投影：−d 方向を見た正面図（x−h平面）／x を右、h を上として描く',
