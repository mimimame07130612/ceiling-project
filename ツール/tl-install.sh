...
#   見出し番号省略 = 全文 / ? = 見出し一覧（L<行番号> <見出し> の形）
#   見出し指定は 見出し番号（例: 3）または 行番号（例: L16）/ ex = 強制取得
...
def tok(t): return re.split(r'[\s\u3000.．、:：]', t.lstrip('#').strip(), 1)[0]
def rng(j):
    i = heads[j]; en = len(lines)
    ...
    return i, en
if num == '?':
    body = '\n'.join('L%d %s' % (i+1, lines[i][:32]) for i in heads)
elif num:
    m = re.fullmatch(r'[Ll](\d+)', num)
    if m:
        js = [j for j, i in enumerate(heads) if i+1 == int(m.group(1))]
    else:
        js = [j for j, i in enumerate(heads) if tok(lines[i]) == num] or \
             [j for j, i in enumerate(heads) if lines[i].lstrip('#').strip().startswith(num)]
    if not js:
        print('NO_SECTION', num, '(? 一覧のL番号か見出し番号を指定)'); raise SystemExit(1)
    st, en = rng(js[0])
